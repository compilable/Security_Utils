...
from PyQt6.QtWidgets import QApplication, QRadioButton, QButtonGroup, QLabel, QComboBox, QFrame, QWidget, QPushButton, QListWidget, QVBoxLayout, QFileDialog, QHBoxLayout, QLineEdit, QCheckBox
...
class FileSelector(QWidget):
    ...
    def init_ui(self):
        ...
        self.combo_box.addItems([str(i) for i in range(1, 11)])  # Adding numbers 1 to 10
    ...
    def generate_passwod(self):
        ...
        iterations = self.combo_box.currentText()
        # 1. generate the hash of each file (iterate by number of times)
        final_pword.append(HasUtils.gen_file_hash(sorted(self.selected_files), selected_hash, iterations))
        
        # 2 generate the hash of the q1,q2,q3 (if exists) (iterate by number of times)
        final_pword.append(HasUtils.gen_question_hash(self.__get_questions(),selected_hash, iterations))
        
        # 3 use the password as the HAMC to generate the final hash of the combind hashs (files + q1 + q2 + q3)
        self.final_password.setText(HasUtils.get_hmac_digest(selected_hash, final_pword,self.password_input.text()))

    def __get_questions(self):
        answer_list=[]
        if self.q1.text() != '':
            answer_list.append(self.q1.text())
        if self.q2.text() != '':
            answer_list.append(self.q2.text())
        if self.q3.text() != '':
            answer_list.append(self.q3.text())
        return answer_list
